...
func New(seed uint64) *Xoshiro256plusplus {
	m := new(Xoshiro256plusplus)
	m.Seed(seed)
	return m
}

// Seed reinitialises the xoshiro256++ random number generator with the given seed
func (r *Xoshiro256plusplus) Seed(seed uint64) {
...
func (r *Xoshiro256plusplus) Uint64() uint64 {
	x := r.s[0] + r.s[3]
	result := ((x << 23) | (x >> 41)) + r.s[0]
	t := r.s[1] << 17
	r.s[2] ^= r.s[0]
	r.s[3] ^= r.s[1]
	r.s[1] ^= r.s[2]
...
func (r *Xoshiro256plusplus) Jump() {
...
func (r *Xoshiro256plusplus) Longjump() {
